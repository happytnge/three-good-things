...
### 問題
メールアドレスが他のユーザーに表示されていた。

### 修正内容
すべての公開ページでメールアドレスを非表示にし、表示名のみを表示するように変更。

#### 更新ファイル:
1. **`src/components/entries/EntryCard.tsx`**
   - 著者情報からメールアドレス削除
   - 表示名がない場合は「ユーザー」と表示

2. **`src/app/dashboard/users/[userId]/page.tsx`**
   - 自分のプロフィールの場合のみメールアドレスを表示
   - 他ユーザーのプロフィールではメールアドレス非表示

3. **`src/components/social/UserSearchBar.tsx`**
   - 検索結果からメールアドレス削除
   - 表示名のみ表示

4. **`src/app/dashboard/discover/page.tsx`**
   - おすすめユーザー一覧からメールアドレス削除

5. **`src/components/social/NotificationList.tsx`**
   - 通知の送信者情報からメールアドレス削除

#### 表示ルール:
- ✅ **表示名あり**: 表示名のみ表示
- ✅ **表示名なし**: 「ユーザー」と表示
- ✅ **自分のプロフィール**: メールアドレスも表示（自分のみ確認可能）
- ✅ **他ユーザーのプロフィール**: メールアドレス非表示
...
### 概要
画像付きエントリーがある日付に、カレンダー上で画像のサムネイルを表示する機能を追加。

### 実装内容

#### 更新ファイル:
1. **`src/components/calendar/CalendarView.tsx`**
   - `entryDates` Set から `entryMap` Map に変更
   - `getEntry()` 関数で日付に対応するエントリー全体を取得
   - CalendarDay に entry オブジェクトを渡す

2. **`src/components/calendar/CalendarDay.tsx`**
   - `hasEntry: boolean` から `entry?: Entry` に変更
   - 画像がある場合の表示:
     - 背景に薄く画像を表示（opacity: 30%）
     - 日付テキストに白い背景を追加（読みやすさ向上）
     - 小さなサムネイル画像（32x32px）を日付の下に表示
   - 画像がない場合:
     - 従来通り青い点を表示

#### デザイン仕様:
- **画像あり**:
  - 背景画像: opacity 30%、全体にフィット
  - 日付: 白背景付き、読みやすく強調
  - サムネイル: 32x32px、白枠付き、シャドウあり

- **画像なし**:
  - 青い点のみ表示（従来通り）

#### 視覚的効果:
- ✅ カレンダーを一目見ただけで、どの日に画像があるかわかる
- ✅ 画像の内容をプレビューできる
- ✅ 日付の視認性を保ちながら画像を表示
- ✅ レスポンシブデザイン対応
